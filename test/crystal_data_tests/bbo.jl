...
# Test sampled phasematches (from: Handbook of Nonlinear Crystals) and D.N. Nikogosyan: "Beta Barium Borate (BBO) - A Review of Its Properties and Applications", (1991)
...
@test isapprox(abs(pm1.d_eff), 1.62u"pm/V", rtol=0.2) # From Nikogosyan
...
@test isapprox(abs(pm2.d_eff), 1.37u"pm/V", rtol=0.2) # From Nikogosyan
...
@test isapprox(abs(pm4.d_eff), 0.56u"pm/V", rtol=0.2) # From SNLO
...
@test isapprox(abs(pm5.d_eff), 0.93u"pm/V", rtol=0.2) # From Nikogosyan
