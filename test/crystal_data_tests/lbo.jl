...
# @test isapprox(pm2.eff_data.d_eff, LBO.d_abc[3,1] * (cos(pm2.theta_pm))^2 + LBO.d[3,2] * (sin(pm2.theta_pm))^2, rtol=0.1)

pm3 = find_nearest_pm_along_theta_phi(90u"°", 21.11u"°", (:hi, :hi, :lo), LBO; lambda_r1=1.3188u"µm", lambda_b=0.4396u"µm", temp=293u"K")
# @test all(isnothing.(pm3.o_or_e_rrb)) || all(pm3.o_or_e_rrb .== (:o, :o, :e))
...
# @test isapprox(pm3.eff_data.d_eff, LBO.d_abc[3,2] * cos(pm3.phi_pm), rtol=0.1)

pm4 = find_nearest_pm_along_theta_phi(5.1u"°", 0.0u"°", (:lo, :hi, :lo), LBO; lambda_r1=1.3188u"µm", lambda_b=0.6594u"µm", temp=293u"K")
# @test all(isnothing.(pm4.o_or_e_rrb)) || all(pm4.o_or_e_rrb .== (:e, :o, :e))
...
@test isapprox(pm4.walkoff_angle_rrb[1], 0.0u"°", atol=ustrip(u"rad", 0.1u"°")) 
@test isapprox(pm4.walkoff_angle_rrb[2], 0.533u"°", atol=ustrip(u"rad", 0.1u"°")) 
@test isapprox(pm4.walkoff_angle_rrb[3], 0.0u"°", atol=ustrip(u"rad", 0.1u"°")) 
@test isapprox(abs(pm4.eff_data.d_eff), 0.536u"pm/V", rtol=0.1) # From SNLO
...
@test isapprox(pm5.phi_pm, 0.0u"°", atol=ustrip(u"rad", 0.1u"°"))
@test isapprox(abs(pm5.eff_data.d_eff), 0.85u"pm/V", rtol=0.1) # From SNLO