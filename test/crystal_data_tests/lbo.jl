...
# @test isapprox(pm1.d_eff, LBO.d_abc[3,2] * cos(pm1.phi_pm), rtol=0.1)
...
# @test isapprox(pm2.d_eff, LBO.d_abc[3,1] * (cos(pm2.theta_pm))^2 + LBO.d[3,2] * (sin(pm2.theta_pm))^2, rtol=0.1)
...
# @test isapprox(pm3.d_eff, LBO.d_abc[3,2] * cos(pm3.phi_pm), rtol=0.1)
...
@test isapprox(abs(pm5.d_eff), 0.85u"pm/V", rtol=0.1) # From SNLO
